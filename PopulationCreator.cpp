	...
	int numberOfDoctors, int numberOfGraftingDoctors, const std::vector<Position>& lands)
{
	...
	for (i; i < numberOfSickHumans; ++i)
	{
		...
		while (isPerson[temp] == true)
			temp = rand.random();
		people[i] = std::make_unique<Human>(healthOfHumans, true, Position(lands[temp]), i);
		isPerson[temp] = true;
	}
	for (i; i < numberOfHumans + numberOfSickHumans; ++i)
	{
		...
		while (isPerson[temp] == true)
			temp = rand.random();
		people[i] = std::make_unique<Human>(healthOfHumans, false, Position(lands[temp]), i);
		isPerson[temp] = true;
	}
		for (i; i < numberOfDoctors + numberOfHumans + numberOfSickHumans; ++i)
		{
			...
			while (isPerson[temp] == true)
				temp = rand.random();
			people[i] = std::make_unique<Doctor>(Position(lands[temp]), i);
			isPerson[temp] = true;
		}
